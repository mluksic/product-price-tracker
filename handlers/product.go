...
func (h *Handler) HandleCreateProduct(w http.ResponseWriter, r *http.Request) {
	err := r.ParseForm()
	if err != nil {
		templ.Handler(components.ItemCreatedAlert(false, fmt.Sprintf("There was an issue parsing the form - %s", err.Error()))).ServeHTTP(w, r)
		return
	}
...
func (h *Handler) HandleProductDeletion(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		templ.Handler(components.ItemCreatedAlert(false, fmt.Sprintf("Unablet to delete product: %s", err.Error()))).ServeHTTP(w, r)
		return
	}

	templ.Handler(components.ItemCreatedAlert(true, "You've successfully deleted product")).ServeHTTP(w, r)
}
